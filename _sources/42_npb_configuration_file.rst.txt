...
have the usual 26 blank spaces left padding or not. Usually these blank spaces
are recommended but sometimes due to long kernel names the resulting line length
can exceed the maximum recommended length of 80 characters. In such case it is
better to remove this left padding. This can be done by setting the element
``<padding>`` to ``False``. The default value, even if not defined is ``True``::
...
will result in a MK as follows::
...
whereas the with "padding" set to "True" or not defined::
