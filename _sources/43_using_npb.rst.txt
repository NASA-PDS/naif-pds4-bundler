...
provided in section :ref:`43_using_npb:Product Diff files`.
...
Product Diff files are HTML files that provide a side to side comparison of
...
PDS Validate Tool Configuration File
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

NPB will write a PDS Validate Tool (``validate``) configuration file for convenience
of the user. The options provided by the configuration file are the ones
recommended by NAIF. Namely:

   * ``verbosity`` is set to ``1`` for ``validate`` to provide all ``INFO`` level messages.
     Some tabular data --checksum and ORBNUM products-- content checks are only
     provided as ``INFO`` messages

   * ``skip-context-validation`` is set to ``true`` and indicates validate to
     ...
     consideration. Feel free to remove this argument or set it to ``false``

   * ``rule`` set to ``true`` ensures that the bundle will be validated
     against the schema

   * ``schema``, the PDS IM Schema has to be explicitly provided to
     ``validate`` for it to check it as it would do with any other
     dictionary. This was not the case for older versions of validate

   * ``schematron``, as per ``schema``, the schematron has to be explicitly provided

   * ``strict-field-checks`` is set to ``true`` for validate to provide check the
     tabular data --checksum and ORBNUM products--

   * ``target`` simply specifies the location of the bundle

   * ``report`` simply specifies the location of the resulting validation report

The following ``validate`` example command for ExoMars2016::
...
Would be equivalent to the following resulting ``validate`` configuration file::
   ...
   validate.verbose = 1
   validate.skipContextValidation = true
