...
needed to provide ephemerides (SPKs), planetary constants (PCK) and
shape (PCK or DSK) of targets, trajectory (SPK) and orientation (CK) of the
...
included in the archive *not applicable for new missions*.
...
included in the archive:
  ...
  * SPKs
       ...
       *  latest ground station locations SPK file(s); if any
  ...
  * EK (Note: the EK subsystem is not used on modern missions.)
...
produce kernels of all types (except EKs and maybe DSKs) and most of these
...
these points may help to make this selection:
  ...
  * For the kernel types that don't cover specific time intervals,
    ...
    -- such as planetary, satellite, structures SPK, DSK, LSK,
...
-- it is best to keep the file size under 200-300 MB.
...
All Kenel Types with * next to them are not applicable for new archives.
...
The generation of MKs can be automatized by providing the appropriate
parameters in the NPB configuration file. This is described in detail in
...
SPICE archives have one DOI for the bundle.
...
We recommend following this file name scheme for release plan files::
...
you use the previous release plan as the starting point for the next release
plan (copy the previous one and update it.)
