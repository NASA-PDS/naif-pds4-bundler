...
parentheses is briefly explained below:
    ...
    1. *All SPICE data are archived as UNIX text and binary kernels,
       ...
       utility program named BINGO, available on the "Utilities"s
    ...
    2. *All SPICE data for a given mission get archived in a single
       ...
       ``sc_spice`` where ``sc`` is the spacecraft acronym. For example
    ...
    3. *All SPICE data sets have the following PDS-compliant directory
       structure*::
          ...
          <sc>_spice           bundle root directory
           ...
           |
           |-- miscellaneous   contains the miscellaneous collection 
           | |                 subdirectories
       ...
       ORBNUM files (see the MAVEN bundle for an example.)
    ...
    4. *All SPICE data have the following meta-information files, most
       ...
       MAVEN it is ``maven_spice``). Elements with ``???`` have oone file  
       per archive release.
    ...
    6. *All SPICE archives include a SPICE Description document that provides
       all the required information to describe in detail the SPICE archive.*
    ...
    7. While experts on PDS standards can (and did during
       ...
       polished over 10+ years of creating PDS3 SPICE data sets and over
       ...
       package provided with this document.

Once in the archive, no files are removed from it or replaced with different 
versions with the same name. Instead, new files superseding already archived 
files are added to the archive. For kernels, they are given distinct names and 
the fact that they supersede previously archived kernels is reflected in the 
meta information files (``spiceds_v???.html'' and meta-kernels).