...
.. IMPORTANT::
    Keep all release byproducts for all releases in the working
    directory! This is important for NPB to generate the correct release
    number and do necessary checks. It is also important to keep these files
    for history of what was run and what was generated by NPB.

Running NPB in a nutshell
=========================

Write/update the following files
    - configuration file (ex. mars2020_release_01.xml)
    - release plan (ex. mars2020_release_01.plan)

Drop into the NPB Virtual Environment::

    % source ~/virtenvs/npb/bin/activate.csh
    % source /mnt/c/Users/abailey/virtenvs/npb/bin/activate.csh

(optional) Run a test/staging run - does not make the final bundle::

   % naif-pds4-bundler working/insight_release_01.xml -p working/insight_release_01.plan -l -v -d files -f staging

    1. Check the log file for warnings and errors
    2. Look at the diff_* files on a web browser
    3. Check staging/
    4. Determine if the NPB config, plan, etc.. need updating before running a final run.

* Clear the staging run files::

   % naif-pds4-bundler working/insight_release_01.xml -c working/insight_release_01.file_list -l -v

* Delete the following from working/--
    - insight_release_01.file_list
    - insight_release_01.kernel_list
    - insight_release_01.log
    - insight_release_01.validate_config
    - diff_*

    *Be sure to keep the NPB config xml and plan that you created!*

    **At this point staging/ and working/ should be cleared of any NPB artifacts**

    Run as many staging runs as needed!

Run a final run - makes the final bundle::

   % naif-pds4-bundler working/insight_release_01.xml -p working/insight_release_01.plan -l -v -d files
