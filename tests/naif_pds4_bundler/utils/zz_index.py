...
class TestIndex(TestCase):
    ...
    def test_pds3_msl_index(self):
        ...
        if not equal:
            raise Exception("index.tab and data/msl_index.tab not equal")
        equal = filecmp.cmp("dsindex.tab", "../data/msl_dsindex.tab")
        if not equal:
            raise Exception("dsindex.tab and data/msl_dsindex.tab not equal")
        equal = filecmp.cmp("index.lbl", "../data/msl_index.lbl")
        if not equal:
            raise Exception("index.lbl and data/msl_index.lbl not equal")
        equal = filecmp.cmp("dsindex.lbl", "../data/msl_dsindex.lbl")
        if not equal:
            raise Exception("dsindex.lbl and data/msl_dsindex.lbl not equal")
