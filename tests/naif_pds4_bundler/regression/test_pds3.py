    ...
    for product in all_files:
        if os.path.isfile(product):
            ...
            with open(product) as ff:
                ...
                fromlines = [
                    item for item in fromlines if ("PRODUCT_CREATION_TIME" not in item)
                ]
            ...
            with open(test_product) as tf:
                ...
                tolines = [
                    item for item in tolines if ("PRODUCT_CREATION_TIME" not in item)
                ]
...
def test_msl(self):
    ...
    shutil.copy2(
        f"../data/spiceds_{self.mission}.cat",
        f"staging/{self.volid}/catalog/spiceds.cat",
    )
    shutil.copy2(
        f"../data/release_{self.mission}.cat",
    ...
    shutil.copy2(
        "../data/msl_release_00.plan", f"working/msl_release_{self.release}.plan"
    )
