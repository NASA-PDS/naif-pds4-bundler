...
def clear_run(setup: Setup) -> None:
    ...
    if os.path.isfile(setup.args.clear):
        ...
        #
        path = setup.staging_directory
        logging.info(f"-- Removing files from staging area: {path}.")
        with open(setup.args.clear, "r") as c:
            for line in c:
        ...
        #
        path = setup.bundle_directory + os.sep + setup.mission_acronym + "_spice/"
        ...
        if setup.args.kerlist:
            try:
                byproduct = setup.args.clear.split(".")[0] + ".plan"
        ...
        if setup.args.plan:
            try:
                byproduct = setup.args.clear.split(".")[0] + ".kernel_list"
                logging.info(f"-- Removing previous run by-product: {byproduct}.")
                os.remove(path + os.sep + byproduct.split(os.sep)[-1])
            except:
                logging.warning(f"     File {byproduct} not found.")
