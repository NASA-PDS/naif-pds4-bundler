...
from naif_pds4_bundle.utils import etree_to_dict
from naif_pds4_bundle.utils import kernel_name
...
class Setup(object):
    ...
    def clear_run(self, debug=False):
        ...
        if debug:
            logging.warning(f'-- Running in DEBUG mode, intermediate files'
                            f' are not cleaned up.')
        ...
        if os.path.isfile(self.args.clear):
            ...
            #
            path = self.final_directory + os.sep + self.mission_acronym + \
                   '_spice/'
            ...
            logging.info(f'-- Trying to remove existing release kernel list '
                         f'from: {path}.')
            try:
                os.remove(path + os.sep + self.args.clear.replace('.file_list',
                          '.kernel_list'))
            except:
                logging.error('     File {} not found.')
            
        else:
            error_message(f'The file provided with the "clear" argument does '
    ...
    def write_file_list(self):
        ...
        if self.file_list:
            with open(self.working_directory + os.sep +
                      f'{self.mission_acronym}_release_'
                      f'{int(self.release):02d}.file_list', 'w') as l:
